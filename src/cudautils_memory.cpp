...
{
  cv::Mat image = cv::Mat::zeros(iImgSize, cv::DataType<imgT>::type);
  ...
  //--- downlaod data ---
  checkCudaErrors(
        cudaMemcpyFromArray(image.data, iCuArray, 0,0, iImgSize.width * iImgSize.height * sizeof(imgT),
                    cudaMemcpyDeviceToHost));
...
template cudaTextureObject_t uploadImageToTexture<uchar>(const cv::Mat&, const bool);
