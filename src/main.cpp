...
int main(int argc, char *argv[])
{
  ...
  //--- read input image ---
  cv::Mat inputImg = cv::imread(positionalArgs[0].toStdString());
  cv::Mat inputImgRGB;
  cv::cvtColor(inputImg, inputImgRGB, CV_BGR2RGBA);
  cv::Mat outputImg = runCudaGrayscaleCvt(inputImgRGB);

  //--- if output path is set write image into file ---
  if(cmdParser.isSet("o"))
  {
    QString outputPath = cmdParser.value("o");
    cv::imwrite(outputPath.toStdString(), outputImg);
  }
  ...
  cv::imshow(inputWinName, inputImg);
  cv::imshow(outputWinName, outputImg);
  cv::waitKey();
